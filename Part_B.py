...
def NaiveBayes(airline, origin, destination):
    ...
    answer = [0.5, 0.5] # Initializing an answer list with two constants [P(Y),P(N)]
    probList=[]
    for i in range(3): # fills in the list of probabilities for each argument
        counterList=[]
        for line in flightList:
    ...
    for element in arguments: # Iterates through each argument and evaluates their P(xi|Ck)
        ...
        tempYes = 0
        tempNo = 0
        for line in flightList: # Appends desired rows into a temporary list, fails if arguments appear in other columns
            if element in line:
